...
post_id = []
raw_text = []
with open('../lsh_data/day_902_903.data') as comment_file:
	for line in comment_file:
		post_id.append(line.strip().split('$&&$')[0])
		raw_text.append(line.strip().split('$&&$')[1])
print 'post_id  and raw_text length : ',len(post_id),len(raw_text)
...
print 'hash_post_id and hash_code length :' len(hash_post_id),len(hash_code)
...
with open('hash_code_raw_text','w') as hash_code_raw_text:
	res = []
	for elem in info:
